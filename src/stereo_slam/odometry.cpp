...
Odometry::Odometry() {
    gftt_ = cv::GFTTDetector::create(400, 0.01, 25);
    num_features_init_ = 80;
    num_features_ = 400;
}
...
void Odometry::SetObservationsForKeyFrame() {
...
inline bool Odometry::triangulation(const std::vector<SE3> &poses,
...
int Odometry::TriangulateNewPoints() {
...
int Odometry::EstimateCurrentPose() {
    // setup g2o
    ...
    for (size_t i = 0; i < current_frame_->features_left_.size(); ++i) {
        ...
        if (mp) {
            ...
            cv::Point2f p = current_frame_->features_left_[i]->position_.pt;
...
int Odometry::FindFeaturesInRight() {
    // use LK flow to estimate points in the right image
...
bool Odometry::Reset() {
    std::cout << "Reset is not implemented. ";
    return true;
}
